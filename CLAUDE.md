...
- **寄送 email**：目標價達成只在畫面上顯示「已達目標價」狀態，不接任何 SMTP 或寄信服務。
  ⚠️ `frontend/app/watch/page.tsx` 目前有三處文案仍寫著「email 通知」（第 32、64、178 行），
  與這條相牴觸。那是還沒修掉的假承諾，**不是**可以據此去接寄信服務的依據。
  同一頁第 178 行還寫著「價格為示意資料」，那也已經過時——五家全部是真實取價。
...
| 搜尋頁有「價格上限」滑桿 | 已移除（後端與前端的 `maxPrice` 都保留） | 依使用者要求；價格區間改由進階搜尋負責 |
...
│   │            另有 app/api/lookups：找書刻意走 Route Handler 而非 Server Action，
│   │            後者與路由導覽共用佇列，找書在飛時整頁的連結都會沒有反應
│   ├── components/
│   └── lib/     後端 API 用戶端、session、設計 token
├── design/      設計稿與設計系統（唯讀參考，不要改；不在版控）
├── docs/        ADR、agent 說明、通路調查（**不在版控**，只在開發者本機）
├── CONTEXT.md   領域語言的詞彙表（作品／版本／通路／報價／找書／追蹤…）
└── start-dev.ps1  一次啟動前後端
```

開發時後端跑 `http://localhost:8080`，前端跑 `http://localhost:3000`。
根目錄的 `.\start-dev.ps1` 會各開一個視窗把兩邊都起起來；連接埠已被佔用時它只報出
佔用的行程與 PID 就中止，不會自動 kill。
...
- 誠實的 User-Agent，同一站台請求最小間隔 `min-interval-ms`（目前 600 毫秒，分主機計算）。
  五家的 robots.txt 都沒有宣告 `Crawl-delay`，所以這是我們自訂的下限；哪天有通路宣告了，
  那家該遵守它自己的值，作法是改成分主機設定，不是把全體拉回去。
